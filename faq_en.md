...
<br>
To ensure that the announcement is published on TED on the same day, we recommend that the announcement is submitted to the mediation service by 23:00 on the same day.

Example for the FVH:
 To ensure that the notice is sent to the switching service on the same day, we recommend that the notice be submitted to the specialist procedure manufacturer by 22:00 on the same day.
</details>
<br>
...
You can find a detailed description of how references and corrections to old notices are possible here: https://github.com/EFA-FHB/ozg-vermittlungsdienst-doku/blob/main/documentation/eForms_creation.md
</details>
<br>
...
*Questions and suggestions for improvement* regarding the switching service and the eSender Hub can be directed to oeffentliche-vergabe@nortal.com. Alternatively, Github issues can be created in the documentation repository and the SDK-eForms-DE repository.

For *support requests regarding the operation* of the switching service and the eSender Hub, please contact: support-oeffentlichevergabe@bdr.de.
...
<br>
There are two ways to read the time of successful submission to TED from the status information in the switching service. Either via the technical timestamp when the eSender received confirmation of the submission from the TED API (<tedStatus>ACCEPTED</tedStatus> tedStatusUpdate>2023-07-31T08:02:02.442Z</tedStatusUpdate>) or via the submittedAt value transmitted by TED, which is included in the status query (<ted_accepted_timestamp>). Both values are only slightly different, as there can be a minimal delay between the time when TED accepts the notification and when a success message is returned to the eSender as a response.
</details>
<br>
...
Example 1:<br>
The switching service accepts a notice which is valid according to the switching service/eSenderHub. The EU later rejects acceptance ("rejected" status), for example because the SDK version is no longer supported. The platform operator receives the status "internalError" according to the status mapping. What would the next steps be in this case?<br>
Example 2:<br>
...
- Referral service API parameter: authorEmail
